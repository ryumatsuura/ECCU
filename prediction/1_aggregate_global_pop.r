...
for (i in iso_codes) {
    ...
    out = exact_extract(x = pop_crop, y = spdf, fun = 'weighted_mean', weights = 'area', progress = FALSE)
    spdf$Population = log(out + 1)
    
    ## append spatial data frames
    if (exists('pop_density')) {
        pop_density = rbind(pop_density, spdf)
    } else {
        pop_density = spdf
    }
    
    ## extract mean population raster value at subnational level
    subnat_out = exact_extract(x = pop_crop, y = country, fun = 'weighted_mean', weights = 'area', progress = FALSE)
    ln = log(subnat_out + 1)
    
    ## append spatial data frames
    if (exists('subnat_pop_density')) {
        subnat_pop_density = rbind(subnat_pop_density, data.table('GDLCODE' = country$GDLcode, 'Country' = country$iso_code, 'X' = points$x, 'Y' = points$y, 'Population' = ln))
    } else {
        subnat_pop_density = data.table('GDLCODE' = country$GDLcode, 'Country' = country$iso_code, 'X' = points$x, 'Y' = points$y, 'Population' = ln)
    }
...
## save population density into csv
fn = file.path(data_dir, 'int/applications/population/population_density_global.csv')
write.csv(x = pop_density, file = fn)

## save population density into csv
fn = file.path(data_dir, 'int/applications/population/population_density_global_subnat.csv')
write.csv(x = subnat_pop_density, file = fn)

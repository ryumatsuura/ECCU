...
for country in ['brb', 'lca']:
    ...
    for unit in units:
        ...
        for task in ['population', 'income']:
            ...
            if unit == 'mosaiks' and task == 'population':
                continue
            ...
            if unit == 'mosaiks':
                mosaiks_only = pd.read_csv(os.path.join(c.features_dir, 'Mosaiks_features_{}.csv'.format(country)))
                mosaiks_only = mosaiks_only.set_index(mosaiks_only['Lat'].astype(str) + ':' + mosaiks_only['Lon'].astype(str)).drop(columns = ['Lat', 'Lon', 'BoxLabel'])
            else:
            ...
            for df in (mosaiks_only, nl_only, both):
                ...
                if task == 'population':
                    ...
                    Y_np = np.array(Y.loc[indices, ['ln_income']])

...
for label in labels_to_run:
    ...
    ## load population/nl data
    pop_nl = pd.read_csv(os.path.join(c.data_dir, 'int', 'applications', label, '{}_global.csv'.format(label)), index_col = 0)
    subnat_pop_nl = pd.read_csv(os.path.join(c.data_dir, 'int', 'applications', label, '{}_global_subnat.csv'.format(label)), index_col = 0)
    
    ## set index
    pop_nl = pop_nl.set_index('iso_code')
    subnat_pop_nl = subnat_pop_nl.set_index('GDLCODE')
    
    ## obtain common indices 
    indices = pd.merge(pop_nl, mosaiks_feat, left_index = True, right_index = True).index
    subnat_indices = pd.merge(subnat_pop_nl, mosaiks_subnat_feat, left_index = True, right_index = True).index
    ...
    for l in ['nat', 'subnat']:
        ...
        ## merge x and y
        if l == 'nat':
            Y, X, latlons, ids = parse.merge(pop_nl.loc[indices, [label.capitalize()]], mosaiks_feat[mosaiks_feat.index.isin(indices)], pop_nl.loc[indices, ['X', 'Y']].rename(columns = {'Y':'lat', 'X':'lon'}), pd.Series(pop_nl.index, index = pop_nl.index))
        elif l == 'subnat':
            Y, X, latlons, ids = parse.merge(subnat_pop_nl.loc[subnat_indices, [label.capitalize()]], mosaiks_subnat_feat[mosaiks_subnat_feat.index.isin(subnat_indices)], subnat_pop_nl.loc[subnat_indices, ['X', 'Y']].rename(columns = {'Y':'lat', 'X':'lon'}), pd.Series(subnat_pop_nl.index, index = subnat_pop_nl.index))
        ...
        np.savetxt(os.path.join(c.data_dir, 'int', 'weights', 'global_{}_{}.csv'.format(l, label)), wts, delimiter = ',')

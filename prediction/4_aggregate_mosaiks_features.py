...
def match_mosaiks_poly(shp, mosaiks_feat_gdf, col):
    ...
    return feats, sjoin[col].to_frame()

## create folder if not exists
if not os.path.exists(os.path.join(c.data_dir, 'int', 'keys')):
    os.makedirs(os.path.join(c.data_dir, 'int', 'keys'))
...
for files in os.listdir(c.features_dir):
    if files.startswith('Mosaiks_features') and files.endswith('.csv'):
        ...
        shp = gpd.read_file(os.path.join(c.data_dir, 'raw', 'shp', 'gadm41_{}_shp'.format(ISO), 'gadm41_{}_0.shp'.format(ISO)))
        ...
        ## store MOSAIKS features that fall into each country
        sjoin = gpd.sjoin(mosaiks_feat_gdf, shp, how = 'left', op ='within')
        sjoin = sjoin[sjoin['GID_0'].notna()]
        
        ## append
        eccu_mosaiks_feat = pd.concat([eccu_mosaiks_feat, pd.DataFrame(sjoin.drop(columns = ['total_pop', 'geometry', 'index_right', 'GID_0', 'COUNTRY']))])
        
        ## compute the national average of MOSAIKS features weighted by population
        feat_means = np.average(sjoin.iloc[:, 0:4000], weights = sjoin['total_pop'], axis = 0)
        ...
        subnat_feat, subnat_feat_key = match_mosaiks_poly(shp, mosaiks_feat_gdf, 'GID_1')
        ...
        if unmatched_shp.shape[0] > 0:
            ...
            buffer_subnat_feat, buffer_subnat_feat_key = match_mosaiks_poly(buffers[['GID_1', 'geometry']], mosaiks_feat_gdf, 'GID_1')
            ...
            eccu_subnat_key = pd.concat([eccu_subnat_key, subnat_feat_key, buffer_subnat_feat_key]) 
        
        else:
            subnat_all_feats = subnat_feat.set_index('GID_1')
            eccu_subnat_key = pd.concat([eccu_subnat_key, subnat_feat_key])
        ...
        feat_means_matrix = pd.DataFrame(np.resize(feat_means, (sjoin.shape[0], 4000)), index = sjoin.index)
        mosaiks_all_feats = pd.DataFrame(sjoin.drop(columns = ['total_pop', 'geometry', 'index_right', 'GID_0', 'COUNTRY']))
...
eccu_subnat_key.to_pickle(os.path.join(c.data_dir, 'int', 'keys', 'eccu_subnat_mosaiks_key.pkl'))
...
## spatially match MOSAIKS features and subnational polygons
brb_ed_feat, _ = match_mosaiks_poly(shp, mosaiks_feat_gdf, 'psu')
...
## spatially match again
buffer_brb_ed_feat, _ = match_mosaiks_poly(buffers[['psu', 'geometry']], mosaiks_feat_gdf, 'psu')
...
## spatially match MOSAIKS features and subnational polygons
lca_settle_feat, _ = match_mosaiks_poly(shp, mosaiks_feat_gdf, 'settlement')
...
## spatially match again
buffer_lca_settle_feat, _ = match_mosaiks_poly(buffers[['settlement', 'geometry']], mosaiks_feat_gdf, 'settlement')
